...
function AdminDenuncias() {
  ...
  const getStatusColor = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      default: return 'text-admin-text-admin-purple bg-admin-border-light border-admin-border-light';
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-admin-text-admin-purple text-sm font-semibold mt-1">Monitore e responda às ocorrências do bairro</p>
        </div>
      </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-admin-hover">
                <th className="px-6 py-4 text-[10px] font-semibold text-admin-text-admin-purple">Denúncia</th>
                ...
                <th className="px-6 py-4 text-[10px] font-semibold text-admin-text-admin-purple">Data</th>
            ...
            <tbody className="divide-y divide-admin-border-light">
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-admin-text-admin-purple animate-pulse">Carregando denúncias...</td>
                </tr>
              ) : denuncias.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center text-admin-text-admin-purple">Nenhuma denúncia encontrada</td>
                </tr>
              ) : (
                      ...
                      <div className="flex items-center gap-3">
                        <div className="size-10 rounded-xl bg-admin-border-light border border-admin-border-light overflow-hidden flex items-center justify-center">
                          {denuncia.foto_url ? (
                            ...
                            <ImageIcon size={18} className="text-admin-text-admin-purple" />
                          )}
                        </div>
                        <div className="max-w-xs">
                          <p className="text-sm font-bold truncate">{denuncia.descricao || 'Sem descrição'}</p>
                          <p className="text-[10px] text-admin-text-admin-purple mt-1 ">{denuncia.categoria}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <p className="text-xs font-bold text-admin-text-admin-purple">{denuncia.usuario?.nome}</p>
                      <p className="text-[10px] text-admin-text-admin-purple mt-1">{denuncia.endereco}</p>
                    </td>
                    <td className="px-6 py-4">
                      <span className={cn(
          ...
          <div className="bg-admin-surface border border-admin-border-light w-full max-w-3xl rounded-xl overflow-hidden shadow-lg animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col">
            <div className="p-6 border-b border-admin-border-light flex items-center justify-between">
               <div>
                 ...
                 <p className="text-[10px] text-admin-text-admin-purple font-semibold">Protocolo: {selectedDenuncia.id.split('-')[0]}</p>
               </div>
               <button 
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  ...
                  <div>
                    <h4 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-admin-text-admin-purple  mb-3">Descrição do Cidadão</h4>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div className="bg-admin-border-light p-3 rounded-lg border border-admin-border-light">
                        <p className="text-[10px] text-admin-text-admin-purple font-semibold mb-1">Categoria</p>
                        <p className="text-xs font-bold uppercase">{selectedDenuncia.categoria}</p>
                     </div>
                     <div className="bg-admin-border-light p-3 rounded-lg border border-admin-border-light">
                        <p className="text-[10px] text-admin-text-admin-purple font-semibold mb-1">Status Atual</p>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[10px] text-admin-text-admin-purple font-semibold">Relatado por</p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[10px] text-admin-text-admin-purple font-semibold">Enviado em</p>
                  ...
                  <div className="pt-6 border-t border-admin-border-light">
                    <h4 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-admin-text-admin-purple  mb-4">Atualizar Status</h4>
                         ...
                         <button
                           ...
                           className={cn(
                             ...
                             selectedDenuncia.status === value 
                              ...
                              : "bg-admin-border-light text-admin-text-admin-purple hover:bg-admin-border-light"
                           )}
                         >
                           {label}
                         </button>
            ...
            <div className="p-6 bg-admin-hover border-t border-admin-border-light">
               <div className="relative">
                  <MessageSquare size={16} className="absolute left-4 top-4 text-admin-text-admin-purple" />
                  <textarea 
